...
for i in puzzle:
	if i.split(' ')[0] == 'swap':
		...
		elif i.split(' ')[1] == 'right':
			tekst = tekst[-int(i.split(' ')[2]) :] + tekst[: -int(i.split(' ')[2])]
		else:
			idx = idx_of(tekst, i.split(' ')[6]) + 1
			if idx_of(tekst, i.split(' ')[6]) >= 4:
				idx += 1
			idx %= len(tekst)
			tekst = tekst[-idx :] + tekst[: -idx]
	else:
		if int(i.split(' ')[2]) < int(i.split(' ')[5]):
			tekst = last(tekst, i.split(' ')[2], int(i.split(' ')[5]))
		else:
			tekst = last(tekst, i.split(' ')[2], int(i.split(' ')[5]))
